...
import numpy as np
from sophon import as_variable
...
def plot_dot_graph(output, verbose=True, to_file='graph.png'):
    ...
    # dotデータをファイルに保存
    tmp_dir = os.path.join(os.path.expanduser('~'), '.sophon')
...
cache_dir = os.path.join(os.path.expanduser('~'), '.sophon')


def get_file(url, file_name=None):
    """Download a file from the `url` if it is not in the cache.
    The file at the `url` is downloaded to the `~/.sophon`.
    Args:
